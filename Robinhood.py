...
class Robinhood:
    ...
    def quote_data(self, stock=None):
        ...
        try:
            res = json.loads((urllib2.urlopen(url)).read().decode('utf-8'));
            if len(res) > 0:
                return res;
            else:
    ...
    def get_historical_quotes(self,symbol,interval,span,bounds='regular'):
        ...
        res = self.session.get(self.endpoints['historicals'], params={'symbols':symbol.upper(), 'interval':interval, 'span':span, 'bounds':bounds})
        return res.json()
    ...
    def place_order(self, instrument, quantity=1, bid_price = None, transaction=None):
        ...
        data = 'account=%s&instrument=%s&price=%f&quantity=%d&side=%s&symbol=%s&time_in_force=gfd&trigger=immediate&type=market' % (
            ...
            urllib2.unquote(instrument['url']),
            float(bid_price),
            quantity,
            transaction,
            instrument['symbol']
